...
def send_to_slack():
    ...
    if image_b64:
        try:
            ...
            # Step 1: Post caption as main channel message (resolves channel name → ID)
            ...
            if not url_data.get("ok"):
                return jsonify({"ok": True, "warning": f"Caption sent but upload failed: {url_data.get('error')}"})
